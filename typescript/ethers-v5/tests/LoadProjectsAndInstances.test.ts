...
import {
    EUROPA_ENDPOINT,
    MAINNET_ENDPOINT,
} from "@skalenetwork/skale-contracts/tests/setup";
import { describe, test } from "vitest";
import {
    loadRequirements,
    testAllocator,
    testInstancesForProvider,
} from "@skalenetwork/skale-contracts/tests/common";
...
const getContractAddress = (contract: BaseContract) => contract.address;

describe("Tests loading skale projects and instances", () => {
    describe("Testing instances on Mainnet", async () => {
        const adapter = new Ethers5Adapter(
            new ethers.providers.JsonRpcProvider(MAINNET_ENDPOINT),
        );
    ...
    describe("Testing instances on Schain Europa", async () => {
        const provider = new ethers.providers.JsonRpcProvider(EUROPA_ENDPOINT);
        await testInstancesForProvider(
