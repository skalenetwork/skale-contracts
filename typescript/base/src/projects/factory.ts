...
import { MirageManagerContractName } from "./mirage-manager/mirageManagerInstance";
...
import { ProjectNotFoundError } from "../domain/errors/project/projectNotFoundError";
import { SchainImaContractName } from "./ima/schain/SchainImaInstance";
import { SchainImaProject } from "./ima/schain/SchainImaProject";
import { SkaleAllocatorContractName } from "./skale-allocator/skaleAllocatorInstance";
...
export const createProject = function createProject<ContractType>(
    ...
    switch (name) {
        case SkaleProject.MAINNET_IMA:
            return new MainnetImaProject<ContractType>(network, metadata);
        case SkaleProject.PAYMASTER:
            return new PaymasterProject<ContractType>(network, metadata);
        case SkaleProject.SCHAIN_IMA:
            return new SchainImaProject<ContractType>(network, metadata);
        case SkaleProject.SKALE_ALLOCATOR:
            return new SkaleAllocatorProject<ContractType>(network, metadata);
        case SkaleProject.SKALE_MANAGER:
            return new SkaleManagerProject<ContractType>(network, metadata);
        case SkaleProject.MIRAGE_MANAGER:
            return new MirageManagerProject<ContractType>(network, metadata);
        default:
