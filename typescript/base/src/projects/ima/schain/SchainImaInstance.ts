...
export class SchainImaInstance<ContractType> extends ImaInstance<ContractType> {
    ...
    getContractAddress(name: string): Promise<ContractAddress> {
        ...
        if (SchainImaInstance.PREDEPLOYED.has(name)) {
            return Promise.resolve(
                SchainImaInstance.PREDEPLOYED.get(name) as ContractAddress,
            );
        }
