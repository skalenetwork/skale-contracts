...
import { MetadataIsNotDownloaded } from "./domain/errors/metadata/metadataIsNotDownloaded";
...
export class Metadata {
    ...
    async download() {
        if (typeof this.networksMetadata === "undefined") {
            const metadata = (
                await axios.get(REPOSITORY_URL + METADATA_FILENAME)
            ).data as MetadataFile;
